  ...
  return (
    <DayPicker
      ...
      components={{
        ...
        IconRight: ({ className: cls, ...rest }: any) => (
          <ChevronRight className={cn("h-4 w-4", cls)} {...rest} />
        ),
        Dropdown: ({ value, onChange, children, ...rest }: any) => (
          <select
            value={value}
            onChange={onChange}
            className="bg-background border border-input rounded-md text-sm px-2 py-1 cursor-pointer focus:outline-none focus:ring-1 focus:ring-ring"
            {...rest}
          >
            {children}
          </select>
