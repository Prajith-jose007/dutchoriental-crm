...
import { Wine, Plus, Minus, ShoppingCart, X } from "lucide-react";
...
export default function BarPOS() {
  ...
  const barCategories = categories.filter(c => c.type === 'bar' || c.type === 'both');
  const barItems = items.filter(item => {
    const category = categories.find(c => c.id === item.category_id);
    return category && (category.type === 'bar' || category.type === 'both');
  });
    ...
    <div className="p-4 md:p-8 bg-gradient-to-br from-gray-50 to-purple-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Bar POS</h1>
            <p className="text-gray-600">Point of Sale for bar orders</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="text-right">
              <p className="text-sm text-gray-600">Cart Items</p>
              <p className="text-2xl font-bold text-purple-700">{cart.length}</p>
            </div>
            <Button
              onClick={() => setIsCheckoutOpen(true)}
              disabled={cart.length === 0}
              className="bg-gradient-to-r from-purple-600 to-purple-700 hover:shadow-lg"
            >
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {barItems.map((item) => (
                    <Card
                      key={item.id}
                      className="cursor-pointer hover:shadow-md transition-all border-2 hover:border-purple-500"
                ...
                {barItems.length === 0 && (
                  <div className="text-center py-10">
                    ...
                    <p className="text-gray-500">No bar items available</p>
                  </div>
                )}
              </CardContent>
            </Card>
