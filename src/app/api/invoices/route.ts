...
export async function GET(request: NextRequest) {
  try {
    ...
    const sortedInvoices = [...invoices_db].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    return NextResponse.json(sortedInvoices, { status: 200 });
  } catch (error) {
    console.error('Failed to fetch invoices:', error);
    return NextResponse.json({ message: 'Failed to fetch invoices', error: (error as Error).message }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Basic validation
    if (!newInvoice.id || !newInvoice.leadId || !newInvoice.clientName || !newInvoice.amount || !newInvoice.dueDate || !newInvoice.status || !newInvoice.createdAt) {
    ...
    if (existingInvoice) {
      return NextResponse.json({ message: `Invoice with ID ${newInvoice.id} already exists.` }, { status: 409 });
    }
    ...
    return NextResponse.json({ message: 'Failed to create invoice', error: (error as Error).message }, { status: 500 });
  }
}
